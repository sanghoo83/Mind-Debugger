...
const INTERP_MARKERS = [
  ...
  { re: /(무시|싫어|미워|못\s?믿|의심|감시|비꼬|우습게|호구|깔보|관심\s?(이|도)?\s?없|비난|부정한|지루|소홀|별로라|안\s?좋아)/, kind: 'mind-reading' },
  { re: /(의도적|일부러|날\s?향|나\s?때문|내\s?탓|왜\s?나만|나\s?혼자|내가\s?(부족|잘못|망쳤|게을|소홀|이상|복잡|못해|못한))/, kind: 'personalization' },
  { re: /(끝났|망했|늦었|큰일|좆됐|안\s?될|못\s?할|거야|거다|것이다|할\s?거|죽었|죽은|글렀|거절|기각|안\s?하겠|가망\s?없)/, kind: 'fortune-telling' },
  { re: /(정치|공격|당한다|당할|음모|뒤통수|찍혔|찍힌|밉보|뒷담|버림받|손절|시작이[다야]|큰\s?병|큰\s?문제|칼\s?꽂|이해\s?못)/, kind: 'catastrophizing' },
  { re: /(항상|언제나|맨날|매번|원래|평생|하나도|전혀|또다시|또\s|매사)/, kind: 'overgeneralization' },
  { re: /(분명|틀림없|확실히|뻔|봐도|100%|틀렸|틀린)/, kind: 'certainty' },
  { re: /(나는?\s?(부족|무능|안\s?돼|재능\s?(이|가)?\s?없|쓸모|이상|모자)|나\s?같은|내가\s?잘못|가치\s?(이|가)?\s?없|감각\s?(이|가)?\s?없|체질\s?아니|경쟁력\s?(이|가)?\s?없|쓸모\s?(가)?\s?없|소용\s?(이)?\s?없)/, kind: 'labeling' },
  { re: /(뒤쳐|뒤처|한참\s?멀|뒤떨어|남들?\s?(은|는|보다))/, kind: 'comparison' },
  { re: /(아직\s?멀|아직\s?한참|아직도\s?멀|95%)/, kind: 'perfectionism' },
  { re: /(아닌가|게을러졌나|망쳤나|못했나|소홀했나|뺏겼)/, kind: 'personalization' },
]
const QUOTE_RE = /["“”'']([^"“”'']{1,})["“”'']/
